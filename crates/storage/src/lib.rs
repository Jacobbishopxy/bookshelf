...
impl Storage {
    ...
    pub fn load_settings(&self) -> anyhow::Result<Settings> {
        let row = self
            .conn
            .query_row(
                "SELECT preview_mode, preview_depth, scan_scope, library_roots_json FROM settings WHERE id = 1",
                [],
                |row| {
                    ...
                    Ok((preview_mode, preview_depth, scan_scope, library_roots_json))
                },
            )
            .optional()?;

        let (preview_mode, preview_depth, scan_scope, library_roots_json) = match row {
            Some(value) => value,
            None => ("text".to_string(), 5, "recursive".to_string(), "[]".to_string()),
        };
    ...
    pub fn save_settings(&self, settings: &Settings) -> anyhow::Result<()> {
        ...
        self.conn.execute(
            "UPDATE settings SET preview_mode = ?, preview_depth = ?, scan_scope = ?, library_roots_json = ? WHERE id = 1",
            (
