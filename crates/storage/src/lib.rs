...
use bookshelf_core::{Book, Bookmark, Note, PreviewMode, ReaderMode, ScanScope, Settings};
...
impl Storage {
    ...
    fn migrate(&self) -> anyhow::Result<()> {
        ...
        self.conn.execute(
            "UPDATE settings SET reader_mode = 'image' WHERE reader_mode = 'text' AND preview_mode = 'image'",
            [],
        )?;
        self.conn.execute(
            "UPDATE settings SET preview_mode = 'blocks' WHERE preview_mode = 'image'",
            [],
        )?;
    ...
    pub fn load_settings(&self) -> anyhow::Result<Settings> {
        let row = self
            .conn
            .query_row(
                "SELECT preview_mode, reader_mode, preview_depth, preview_pages, scan_scope, library_roots_json FROM settings WHERE id = 1",
                [],
                |row| {
                    let preview_mode: String = row.get(0)?;
                    let reader_mode: String = row.get(1)?;
                    let preview_depth: i64 = row.get(2)?;
                    let preview_pages: i64 = row.get(3)?;
    ...
    pub fn save_settings(&self, settings: &Settings) -> anyhow::Result<()> {
        ...
        self.conn.execute(
            "UPDATE settings SET preview_mode = ?, reader_mode = ?, preview_depth = ?, preview_pages = ?, scan_scope = ?, library_roots_json = ? WHERE id = 1",
            (
